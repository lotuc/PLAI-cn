...
不使用额外的Racket构造的情况下，显然我们无法直接创建循环数据。我们需要给数据创建一个“地址”，然后在该地址中引用自己。注意这里是用了“然后”，它暗示时间的概念，即我们需要使用赋值操作。这样的话，我们可以试试用`box`来实现。

> 上面提到的额外的Racket构造指的是`shared`，尽管其他语言基本上都没有这个机制，所以这里我们也不深究 `shared`了。我们这里学习的东西正是`shared`幕后实现的基本原理。
