...
在[第五章](./chap5.md)中我们引入函数时并没有特别指明函数定义所在的位置。可以说我们是按照理想化的DrRacket模型引入的函数，即将函数的定义和使用分离。下面我们使用Scheme的设计原则来重新思考一下这种设计的__必要性__。
...
对于数，显然要使用新的返回值类型构造器对其包裹一下。对于标识符，一切不变。对于加法／乘法，需要进行简单的修改使其能正确的返回`Value`类型而不是简单的数字：
