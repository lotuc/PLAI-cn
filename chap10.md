...
对于成员名称的处理我们已经有两个正交的纬度。一个维度是名字是静态给定还是动态计算的，另一纬是名字的数量是固定的还是可变的：
...
截至目前，我们 `case` 表达式中 `else` 部分还是留空的（动态分发部分代码）。一个原因是，如果我们的成员及成员数量可变，那么使用其它方式实现可能是更好的选择：例如上面讨论的哈希表；相对的，对象成员固定时，使用 `case` 条件表达式能更好的服务于演示的目的（因为这种实现方式强调了成员固定这一点，而哈希表则相对开放）。还有一点原因是，我们可以在 `else` 部分移交分发控制权如，给其父对象，这也被成为**继承**。

回到上面实现的对象模型。为了实现继承，需要给对象“一些东西”让其把识别不了的方法代理出去。“那些东西”是什么将导致迥异的设计结果。

现在我们给出的答案可能是在 else 中放另一个对象：
...
当开发者调用 Java 的类构造器时，从效果上来看等同于沿着继承链往上构建对象（现实中，编译器会进行优化使得这个过程只需要进行一次构造器调用和一次对象分配）。这些对象中的每一个都是对应父类的一个针对该构造对象的一个私有拷贝。关于这些对象到底有多少是可见的，Java 的选择是对于一个指定名字（和签名）只保留一个方法，不管该方法在继承链上被实现了多少次，然而所有的字段是都被保留了的，可以通过强制类型转换去获取继承链上特定类的字段值。后者细想是比较合理的，因为每个字段都可能有一些不变量在管理它，所以保证它们彼此分离（也因此所有字段的都存在）是很有必要的。与之相对，很容易想出来一种方式可以使所有方法可用，而不只是最下面那个（通过继承链往上找到的第一个），很多脚本语言使用了这种方法。
...
你可能会想到既然可以传入一个对象描述作为父辈，传入更多个应该也是可以的吧？是的，的确可以，这也很自然的导出了要讨论的多重继承。有多个父辈之后一个很显然的问题是查找方法时按照何种顺序进行。一种比较简单的情况是继承链组织成了树状结构，即使这种比较简单的情况也没有一个权威的顺序可以供你不加思考直接使用：起码要考虑使用深度优先还是广度优先。还有更糟糕的情况，例如 A 扩展自 B 和 C；而 B 和 C 都扩展自 D。这里的问题是：A 的实例中会包含两个 D 对象吗？只包含一个既能节省空间且行为可能更符合我们的期望。但是问题来了，我们应该访问该对象一次还是两次呢？（TODO）

> 这就是臭名昭著的钻石继承问题（diamond inheritance，菱形继承问题）。如果你选择在语言中包含多重继承，关于这个问题涉及的设计抉择可能需要你纠结好长时间。你不大可能找到一个权威的解决方案。

只有当你彻底理解了多重继承的相关问题，才能体会到其魅力。

### 10.3.4 Super-Duper

很多语言中支持 super 调用，如调用继承链上一层的方法或者访问上一层的字段（注意这里说的是“链”，在多重继承的情况下这要复杂的多）。

我们已经对向上查找方法习以为常，但是也许我们忘了问这是否是最自然的方向。【TODO:校正】记住构造器和函数都有访问不变量的能力。应该对谁报以更多信任，超类或是子类？有些人认为子类更为精炼，拥有关于一个对象更多的描述。与之相对，超类能够保护其不变量不受无知的子类胡乱篡改。

这是关于继承到底是什么的两种截然不同的认知。向上意味着我们认为扩展是为了替代超类。向下意味着我们将继承当作对父类做出更细的定义。一般我们将子类继承视为后者，但是为什么我们的语言进行调用的时候却选择了相反的方向呢？因此有些语言探索了默认向下调用。

> [gbeta](http://www.daimi.au.dk/~eernst/gbeta/) 是一门由众多有趣特性的现代语言，它支持 inner。考虑结合这[两个方向](http://www.cs.utah.edu/plt/publications/oopsla04-gff.pdf)也是非常有趣的。

### 10.3.5 Mixin 和 Trait

在 Java 中当我们写下一个类时候，那对大括号中事实上是什么东西呢？它不是那个完整的类：取决它继承的类以及继承的那个类所继承的类，递归向上。即其实我们在那对大括号中写下的其实是一个类的扩展。仅当把其超类考虑进去后它才是一个完整的类定义。

很自然的，我们应该发出这样的疑问：为什么？为什么不把扩展的定义和将该扩展应用于一个基类这两个行为分开呢？即，我们将这段代码：
...
看上去这样好想把我们原来已经实现的东西变复杂了。但是这种类似函数调用的语法不禁让我们浮想联翩：我们可以将该扩展应用于多个不同的基类。即你可以这样做：
...
通过将 E 的定义和其扩展的类分离开，我们将扩展从固定的基类的暴政中解放出来。这种扩展有个名字：mixin。

> mixin 这个名词来源于 Common Lisp，是使用多重继承的一个典型模式。Lipstick on a pig.

mixin 使得类定义具有更好的可组合性。它提供了很多多重继承的好处（重用多段功能代码），但是避免了多重继承的麻烦（例如没有前面讨论的复杂的查询顺序问题）。mixin 可以理解为针对类的函数；我们前面已经将类 desugar 成目标语言中的函数，我们的目标语言中的函数可以接收函数为参数，这使得实现一个简单的 mixin 模型变得非常容易。

在强类型语言（typed language）中，好的 mixin 设计可以改善面向对象编程实践。假设我们要定义一个基于 mixin 的 Java。如果 mixin 是一个等效于类到类的函数，那么这个“函数”的“类型”是什么？显然 mixin 应该使用接口（interface）来描述其参数和结果。

在一个拥有 minxin 的语言中，我们可以这样写：

```java
mixin M extends I { ... }
```

其中 I 是一个接口。这样 M 可以被用来扩展那些实现了接口 I 的类，语言中也能保证接口只有 I 中的成员在 M 中可见。这遵循了一个很好的编程实践。

> “面向接口编程，而不是面向实现（Program to an interface, not an implementation）” —— 《设计模式》

好的 mixin 设计还能走得更远。根据定义，一个类在继承链上只能使用一次（如果一个类的继承链上有它自己，那么该继承链上势必存在环路，导致潜在无限循环）。与之相对的，当我们组合函数时，就不会有这种疑虑（例如：`(map ... (filter (map ...)))`）。可以多次使用一个 mixin 有价值吗？

> 当然有！看看 [Classes and Mixins](http://www.cs.brown.edu/~sk/Publications/Papers/Published/fkf-classes-mixins/) 的第3节和第4节。

mixin 解决了库设计中的一个重要问题。假设我们有一系列可以通过不同方式组合的特性，我们要提供多少类？更甚之，这些特性是不可以随意组合的。显然产生所有需要的组合对应的类不太现实。最好是开发者可以选择自己需要的特性进行组合，且提供了必要的机制防止不合理的组合。这正是 mixin 要解决的问题：mixin 提供面向接口的类扩展，开发者使用它们创建自己需要的类。

**练习**
> 你最喜欢的面向对象语言的库是怎么解决如上问题的？

> Racket 的 GUI 库中广泛使用了 mixin。例如 `color:text-mixin` 接收基本的文本编辑器接口，然后实现一个带颜色的文本编辑器接口。实现的那个接口也是一个基本的文本编辑器接口，额外的文本相关 mixin 还可以继续应用到它上面。

mixin 有一个局限：只能进行线性的组合。这种限制有时会给程序员带来不必要的负担。将 mixin 泛化，不仅支持对单个 mixin 的扩展，还能扩展一组 mixin，这得到的东西被称为 trait 。当然，在扩展多个的时候，必然又会遇到老问题：潜在的名字冲突。因此实现 trait 时必须同时解决名字解析冲突的问题。于是作为对 mixin 的补充，trait 使程序员有了可以定制的满足其需求的功能的技术。Racket 同时提供了 mixin 和 trait。
