...
> __习题__
>
> 请注意，在numC子句，解释器返回n，而替换函数返回in（即原始表达式）。为什么？
...
这可能是我们不太想看到的，因为同一个表达式将会被解释计算两次。你可能会想到，我们可以先计算参数的结果在对参数结果执行搜索替换操作。这样的话，搜索替换函数的类型应该是：

```racket
; subst : number * symbol * ExprC -> ExprC
```

请注意，我们不能直接把数字放入表达式，而是必须先把数字放入numC调用中。所以，替换函数可以用一个辅助函数（事实上，现有的subst函数类型就是对的，它接收的第一个参数类型是ExprC，那么当数据是numC类型时也应当可以工作。）

> __习题__
>
> 修改前述的编辑器，用答案而不是表达式替换标识符。
