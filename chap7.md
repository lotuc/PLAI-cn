...
为什么函数的定义不可以也是一种表达式呢？我们现在实现的算术语言中有一个尴尬的问题：“函数的定义表示的是什么值？”，在现有设计中没有很好的答案。对于真正的语言来说，计算结果当然不可能只有数，所以也没必要给我们的语言作出这种限制；跳出这个框框，便可以给出很好的回答：“函数值”。让我们试试如何实现它。

将函数作为值，能用它做什么呢？显然，函数和数是不同类型的值，你不能对函数做加法运算。但是，有一件它显然能做的事：传入参数调用它！因此我们应该允许函数值出现在函数调用那个位置。其行为，显然是调用该函数。因此，我们的语言中应该允许如下的表达式作为合法程序（这里使用方括号以方便阅读）：
...
(define (num+ [l : Value] [r : Value]) : Value
  (cond
    ...
    [else
     (error 'num+ "one argument was not a number")]))  ;有一个参数不是数
```
