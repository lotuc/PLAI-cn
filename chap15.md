...
现在我们已经有了一个基础的静态类型语言，下面探索一下如何将其扩展成为一个更有用的编程语言。
...
事实上，上面任何两个都不太一样。但是第一个和第三个非常相似，因为第一个是`Java`代码而第三个是我们的静态语言代码，而第二个，是`C++`代码，和其它两个不同。清楚了吗？不清楚？很好，继续往下读！

#### 15.3.1.1 参数化类型

我们使用过的编程语言已经展示过参数多态的价值，例如，`map`函数的类型可以这样给出：

``` Racket
(('a -> 'b) (listof 'a) -> (listof 'b))
```

他的意思是对于任意类型`'a`和`'b`，`map`接受一个从`'a`到''b'的函数，一个`'a`的列表，生成对应的`'b`的列表。这里，`'a`和`'b`不是具体类型；它们是**类型变量**（我们的术语中，这应该被称为“类型标识符”，因为它们在实例化过程中不会发生改变；但是我们还是使用传统术语）。

可以换种方式理解它：实际上有一族无穷多的这样的`map`函数。例如，其中一个的类型是这样的：

``` Racket
((number -> string) (listof number) -> (listof string))
```

还可以是这样的（没有限制说其中的类型必须是基本类型）：

``` Racket
((number -> (number -> number)) (listof number) -> (listof (number -> number)))
```

也可以是这样的（也没有限制说`'a`和`'b`必须不同）：

``` Racket
((string -> string) (listof string) -> (listof string))
```

以此类推。由于它们的类型不同，需要不同的名字：`map_num_str`，`map_num_num->num`，`map_str_str`，等。但是这会让它们变成不同的函数，于是我们总是需要使用一个特定`map`，而不是直接使用一个比较一般的函数。

显然，不可能将所有这些函数放到我们的标准库中：毕竟它们有无穷多个！更好的方式是能按需获取我们需要的函数。我们的命名规则给出了一点提示：`map`接受两个类型参数。通过给定一堆类型作为参数，我们可以获取到针对特定类型的`map`函数。这种**类型的参数化**也被称为**参数多态**。

> 注意不要和对象“多态”搞混，后面会讨论它【引用】。

#### 15.3.1.2 显式声明类型参数
换句话说，我们相当于说`map`实际上是一个有四个参数的函数，其中两个是类型，另外两个是实际的值（函数和列表）。在一个需要显式声明类型的语言中，我们需要写成类似这样：
...
但是这会产生一些问题。首先，`???`处应该填什么？它是类型`a`和`b`的类型。但是`a`和`b`自己需要被类型替换的，所以类型的类型是什么呢？其次，我们真的希望每次调用这个函数的时候需要传递四个参数吗？再者，我们真的希望在接收任何实际值之前就接收类型参数吗？对于这些问题的答案能延伸出关于多态类型系统巨大的讨论空间，其中的大部分我们这里将不会涉及。

> 推荐阅读《Pierce's Types and Programming Languages》，获取比较易懂、现代的介绍。

注意到一旦我们开始参数化，很多预期的代码都将需要被参数化。例如，考虑平平无奇的函数`cons`的类型。它的类型需要基于列表中值的类型进行参数化（尽管它实际上并不依赖于这些值——关于这点参见列表及cons的实现原理）。于是`cons`每次使用时都需要保证类型的正确性。因为这，即使`empty`都需要适当实例化以创建对应类型的空列表！当然，`Java`和`C++`程序员应该对这个痛点很熟悉了。

#### 15.3.1.3 一阶多态
我们将把自己局限在这个巨大讨论空间的一些特别有用和易于理解的点上，也即 Standard ML 的类型系统、同时是本书使用的静态类型语言和早期版本的 Haskell 的类型系统，有范型加成的 Java 和 C# 以及引入了模版的C++ 也差不多获得了这种类型系统的大部分能力。这类语言定义了被称为**谓词**、**一阶**或者叫**前缀**多态的东西。关于上小节的问题它的答案是不填、否，是。下面我们来探讨一下。

我们首先将类型的世界分成两组。第一组包含我们目前用到的静态类型语言，另外加上类型变量；它们被称为 monotype。第二组，也被称为 polytype ，包含参数化类型；按惯例它们是这样写的：一个 ∀ 前缀，一组类型变量，然后一个类型表达式，其中可能用到签名的类型变量。因此，类型`map`将写作：
...
在`rank-1`多态中，类型变量只能被`monotype`替换。（此外，它们只能被具体类型替换，否则剩下的类型变量将无法被替换掉）。因此，在类型变量参数和常规参数之间我们有了一个明确的界线。我们不需要为类型变量提供“类型注解”，因为我们知道它们是什么。这将产生一门相对简洁的语言，但仍提功了相当的表现力。

注意到由于类型变量只能被`monotype`替换，他们全相互对立。于是，类型参数可以全被提到参数表的前面。这使我们可以使用`∀ tv, ... : t`的方式便携类型，其中`tv`是类型变量，`t`是`monotype`（可能引用这些类型变量）。这也是之前称其为前缀多态的原因。而且后面也将看到其对实现也很有用。

> 非直谓性语言（[Impredicative](https://en.wikipedia.org/wiki/Impredicativity) language）擦除了`monotype`和`polytype`的区别，因此其中类型变量可以使用另一个`polytyp`实例化。

#### 15.3.1.4 通过去语法糖实现一阶多态解释器
该特性最简单的实现方式就是视其为一种去语法糖的形式：这实际上是 C++ 的做法。（详细点说，因为 C++ 有一个模版形式的宏系统，通过使用模版，它达成了一阶多态。）举个例子，如果我们有一个语法形式`define-poly`，它接收一个名字、一个类型变量和一个表达式。它将表达式中对应类型变量替换为对于该名字的类型限定，因此：

``` Racket
(define-poly (id t) (lambda ([x : t]) : t x))
```
通过将`id`定义为多态的方式定义了一个恒等（identity）函数：给`t`传递任意具体类型，它返回一个单参数的类型为`(t -> t)`的函数（其中`t`被正确的替换对应类型参数值）。我们可以使用各种类型实例化`id`：
...
如果你好奇的话，下面给出了实现。简单起见，，我们假设只有一个类型参数；很容易使用`...`实现多个的情形。我们将不仅为`define-poly`定义一个宏，它还会定义一个宏：
...
该语言将创建一个名为`id`的宏：对应`(define-syntax (name ...) ...)`的部分（对于这个例子，`name`是`id`）。`id`的一个实例，如`(id number)`，将类型变量`t`，宏里面的`typvar`替换成给定的类型。为规避卫生，我们使用`with-syntax`来确保所有对于类型变量（typvar）的使用被替换为给定的类型。因此，实际效果是，
...
1. 尝试定义一个递归的多态函数，例如`filter`。早先我们说过我们应该使用类型实例化每个多态值（例如`cons`和`empty`），但是为了简洁起见我们将依赖静态类型语言帮我们实现这点，而我们，将仅专注于`filter`的类型参数。下面是对应代码：
  ...
  注意到递归的使用`filter`时，我们需要使用恰当的类型对其进行实例化。
  ...
  DrRacket 将不会终止，更准确的说，是宏扩展不会终止，因为它将不断的尝试创建`filter`**代码的副本**。如果我们使用下面这种方式定义该函数，扩展会终止——
  ...
  但是这给开发人员徒增不必要的痛苦。实际上一些模版展开程序（template expander）将会缓存之前的展开的值，避免对于相同模版再次重新生成代码。（Racket 做不到这点，因为一般来说，宏表达式中可以依赖可变变量和值，甚至可以执行输入输出，因此 Racket 不能保证对于给定的输入表达式总是产生相同输出。）
2. 考虑恒等函数的两个实例。我们不能对`id_num`和`id_str`进行比较，因为它们类型不同，但即使它们类型相同，使用`eq?`比较它们也不同：

  ``` Racket
  (test (eq? (id number) (id number)) #f)
  ```

  这是因为对`id`每次实例化都会重新创建一份新的代码副本。即使使用了上面提到的优化，同一种类型对应代码只有一份副本，但是不同类型的对应代码体还是会被重新生成。——但这是没必要的！例如，`id`的实现的部分其实没什么东西，该实现其实依赖于类型参数。实际上，`id`这一族无穷多个的函数可以共享同一个实现，简单的去语法糖策略实现不了这点。

  > 事实上，`C++`模版因代码膨胀的问题而臭名昭著，这是原因之一。

换句话说，基于去语法糖的策略本质上是使用替换的实现方式，它有着和我们之前函数调用时使用替换的方式实现相同的问题。然而，其它情况下，替换策略能达成我们关于程序行为的期望；对于多态也是一样，正如我们将看到的一样【引用】。

注意去语法糖策略的一个好处就是它不需要类型检查器“理解”多态。我们的核心语言仍可以是单态的（monomorphic），所有的（一阶）多态完全由宏扩展处理。这提供了将多态添加到语言中的一种廉价策略，但正如`C++`中显示的那样，它也引入了很大的开销。

#### 15.3.1.5 其它实现方式

有其它不存在上述问题的实现策略。这里我们不会深入讲解它们，但是其中一些策略的本质就是上面提到过的“缓存”方法。因为可以确定的是，对于给定的同一组类型参数，应该得到相同的实现代码，不需要对相同的类型参数实例化多次。这避免了无限循环。如果我们检查了使用特定类型实例化的代码一次，后续相同类型参数的实例化结果就无需再进行类型检查（因为它不会发生改变）。此外，我们无需保留实例化后的源码：一旦我们检查了扩展后的程序，就可以将其丢弃，运行时也只需要保留一份实例化的副本。这样可以避免上述纯去语法糖策略中讨论过的所有问题，同时保留它的好处。

其实，我们有点太过分了。静态类型的好处之一就是让我们能够选择更精确的运行时表示。例如，一个静态类型可以告诉我们用的是数是32位的还是64位的甚至1位的（实际上，是布尔值）。然后，编译器可以利用位的布局方式（例如，32个布尔值可以打包成一个32位字）为每种表示生成专用代码。因此，在对每种使用的类型进行检查之后，多态实例化程序可以跟踪函数或数据结构使用时用到的特定类型，并将这些信息提供给编译器用于代码生成。这会导致生成相关函数的若干副本，彼此都互不`eq?`——但有充分的理由，因为它们要执行的操作的确不同，所以这是正确的。

#### 15.3.1.6 关系参数

我们还需解决关于多态还有最后一个细节。

早先我们说过像`cons`这样的函数不依赖于其参数的具体值。`map`、`filter`及其它很多函数也这样。`map`和`filter`接收一个函数作为参数，当它们要对单个元素进行操作时，实际上使用该函数进行实际操作，即该函数负责做出如何处理该元素的决定；`map`和`filter`只需遵守该函数参数。

“检验”这是否正确的一种方法是替换一些不同类型的值列表及对应的函数作为参数。也就是说假设我们两组值之间有映射关系；我们根据关系和参数函数转换列表元素。问题是，`map`和`filter`的输出结果是否可以通过该关系预测？如果对于某些输入，`map`的输出和关系预测的结果不同，这说明`map`肯定侦测了实际值并根据相关信息做出了处理。但事实上，这不会发生在`map`上，或者说实际上也不会发生在大多标准多态函数上。

遵从这类型关系准则的函数被成为**关系参数**。这是类型赋予我们的另一个非常强大的能力，因为它们告诉我们这种多态函数可以执行的操作有一个很大的限制：它们可以删除、赋值和重排元素，但是不能考察这些元素，也不能对它们进行一些具体操纵。

> 关系参数：参考阅读Wadler的《Theorems for Free!》和Reynolds的《Types, Abstraction and Parametric Polymorphism》。

起初这听起来非常令人印象深刻（它确实是！），但细查，你可能会意识到这与你的经验并不一致。例如，在Java中，多态方法依然可以使用`instanceof`在运行时检查、获得特定种类的值，并相应的改变其行为。这种方法不是关系参数的。事实上，关系参数也能被看作是语言的弱点的表述：它只允许一组有限的操作。（你仍可以检查类型——但不能根据你获取的信息进行相关行动，这让这种检查没有意义。因此运行时系统如果想要模拟关系参数，必须要移除类似`instanceof`及它的替代行为：例如，对一个值进行加一操作捕获异常判断值是一个数。）然而，这是一个非常优雅和令人吃惊的结果，显示了使用丰富类型系统能获得的想打的程序推理能力。
