  ...
  (type-case ExprC expr
             ...
             [eqC (e1 e2)
                  (type-case Result (interp e1 env sto)
                             [v*s (v-e1 s-e1)
                                  (type-case Result (interp e2 env sto)
                                             [v*s (v-e2 s-e2)
                                                  (v*s (boolV (or (and (numV? v-e1)
                                                                       (numV? v-e2)
                                                                       (= (numV-n v-e1)
                                                                          (numV-n v-e2)))
                                                                  (and (boolV? v-e1)
             ...
             [ifC (condition ifResult elseResult)
                  (type-case Result (interp condition env sto)
                             [v*s (v s)
                                  (if (boolV-b v)
                                      (interp ifResult env s)
                                      (interp elseResult env s))])]
...
(testResult '(if true 1 2) (numV 1))
(testResult '(eq? true true) (boolV true))
(testResult '(eq? true false) (boolV false))
(testResult '(eq? 42 42) (boolV true))
(testResult '(eq? (+ 20 22) (+ 20 22)) (boolV true))
(testResult '(eq? 20 (+ (* -1 22) 42)) (boolV true))
(testResult '((lambda x (eq? x 2)) 2) (boolV true))

;; fact
(testResult '((lambda fact
                (seq
                 (setbox fact
                         (lambda n
                           (if (eq? n 0)
