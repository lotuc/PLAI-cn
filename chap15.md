...
“检验”这种情况是否属实的一种方法是，替换不同类型的值链表及对应的函数作为参数。也就是说假设两组值之间有映射关系；我们根据此关系替换链表元素和参数函数。问题是，`map`和`filter`的输出结果是否可以通过该关系预测？如果对于某些输入，`map`的输出和关系预测的结果不同，这说明`map`肯定侦测了实际值并根据相关信息做出了处理。但事实上，这不会发生在`map`上，或者说实际上也不会发生在大多标准多态函数上。
...
这是因为我们通过合一推断出来的类型实际并不是**多态的**。这点很重要：将其称为类型变量不会使你获得多态!类型变量可以在下次使用时合一，彼时，最终得到的还只是单形函数。而真正的多态只有在能真正进行类型变量**实例化**时才会获得。
...
假设我们要构造一个动物园动物的链表，动物有这些种类：犰狳、大蟒蛇等。首先，我们必须先创建一个数据类型：
...
然后创建它的链表：`(listof Animal)`。`Animal`类型表示的是`armadillo`和`boa`的“联合（union）”，不过只能通过创建新类型的方式创建这种联合：比如我们要创建动物和植物的联合，就需要：
...
- 首先处理哨兵值。很多情况下，哨兵本应该被替换为异常，但是在很多语言中，抛出异常的代价巨大。因此开发者倾向于区分真正的异常情况——不应该发生——和正常运行中的预期情况。检查元素是否属于一个链表发现不存在的情况显然属于后者（如果我们已经直到元素是否存在，这个谓词判断就不需要进行）。在后一种情况下，使用哨兵是合理的。
...
(define-type SExp
  ...
  表示s-表达式。如果一个函数希望操作这些项的某个子集——比如数和数的链表——他们将创建一个新的类型，将值转换到该类型，尽管这两个类型的内部表示几乎相同。另一个例子是，考虑CPS表达式的集合。这显然是所有可能表达式的一个子集，但是如果我们要为其创建一个新的类型，我们将不能使用任何已有的可以处理表达式的程序——如解释器。
...
理解递归类型可以被视为和其展开后是等价的就足够了。例如，我们定义数的链表类型为
...
以此类推（同构和等价递归之间的区别正是在相等性的概念上：是定义上的相等性还是同构意义上的）。每一步中，我们将参数`T`替换成整个类型。和递归值一样，它的意思是需要时我们可以“获得另一个”`ConsL`构造。口语化的来说，链表的类型可以写成零或任意多元素的联合；也即包含零个、一个或任意个元素的类型；以此类推。这些类型适用于（且恰恰）数的链表。
