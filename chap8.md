...
（到这里我们又完成了一个解释器！照例，代码在[这里](./src/chap8.1.rkt)）
...
1. 我们的解释器隐含了一个微妙但重要的设计抉择：**求值顺序**。例如，为什么我们不按如下方式实现加法？
   ...
   * 更为重要的是，**这是一个语义上的抉择**。在以前，加法一个分支参数上的计算不会影响另一个分支上的计算结果。而现在，分支上可能会执行 mutation 操作并因此影响到另一分支，因此要使该语言的程序员能预测自己程序的行为，我们必须要选择一个求值顺序。
2. 观察调用规则，可以发现，我们沿途传递着动态 store，如，在计算函数部分和参数部分时传递的那个 store。这种行为跟我们对于环境的要求正好相对。这是个关键区别。store 从其效果上来说，是“动态作用域的（dynamically scoped）”，这是由于它是用于反映计算的历史，而不是用来反映词法上的东西。由于我们已经使用了名词“作用域（scope）”来表示标识符的绑定，这时再用“动态作用域的”来指代 store 可能会造成困惑。于是我们引入一个新的名词 **persistent** 来指代它。

   一些语言中这两个概念混淆不清。例如在 C 语言中，绑定到本地标识符上的值（默认）在栈上分配。然而，这里栈对应于环境，因此它们将随着调用的结束而消失。如果返回值中引用了这些值，那么这个引用将会指向一个未定义地址，向该地址写入值可能导致数据被覆写：C 语言中很大一部分的错误来源于此。问题的关键是，值不应因为退出一个作用域就消失；消失的只应该是该本地作用域中的标识符。

3. 我们已经讨论过两种实现覆写 store 的策略：简单的扩展它（将依赖于 `fetch` 的实现，需要它总是取出最新的绑定值）；或者采用“搜索替换”的方式。后面这种策略有个好处，不会存储那些无用的我们不可能获取到的数据。

   然而我们并没有考虑过内存管理。随着程序的运行，我们可能永久失去访问特定 `box` 的能力，如一个 `box` 仅被绑定到一个标识符，当我们退出该标识符所在的作用域时，将再也不能访问到该 `box`。这些不能被访问到的位置被称为**垃圾**。从概念上来讲，垃圾的地址是那些清除之后对程序求值结果没有任何影响的地址。有很多用于辨别并回收垃圾的策略，通常被称作“垃圾回收”。
...
4. 要注意，计算每个表达式的时候，总是要让后面的计算依赖之前返回的 store 以正确维护执行历史。比如，考虑下面这种 `unboxC` 的实现：
   ...
   > ```
   注意到没有，我们不是从 `s-a` 而是从 `sto` 中搜索值。但 `sto` 反映的是 `unboxC` 未求值之前的执行历史，而没有包含它求值过程中的执行历史，`unboxC` 表达式求值过程中 `store` 可能发生改变吗？当然了！
   > ```scheme
   > (let ([b (box 0)])
...
5. 下面是另一个常见错误：
   ...
   > 练习，回头看看实现的解释器，看看有没有这些 bug，修复它们。
...
6. 注意到我们可以通过使用旧的 store 进行时间回溯：是 mutation 在程序中引入了时间的概念；我们可以通过撤销 mutation 操作来达到回溯的目的。这听起来好像很有趣还有点任性；它有使用场景吗？
   ...
   事实上这就是软件事务内存（Software Transactional Memory, STM）。每条线程维护自己的日志，未提交部分的改变只有自己能看到。每个线程可以维护自己的状态，如果 commit 成功，那么其提交的绑定将通过全局 store 被所有其它线程看到；如果提交失败，也可以很容易的通过 discard 剔除改变。

   STM 提供了一种很直观的处理多线程编程中共享状态的方法。大部分计算机只有一个全局存储，而维护日志成本非常高，且需要花很大精力维护他们。因而，一些硬件架构开始提供对于事务内存的直接支持，这使得日志的创建、维护和提交可以和全局存储一样高效，移除了采用该想法的一个重大阻碍。

   **练习**
   > 修改语言以增加软件事务内存日志的特性。

**练习**
> 一种替代实现策略是在环境中将名字映射到 `boxed Value`，这里我们没有这样做是因为：
>
> 1. 这样做的话有种作弊的感觉
> 2. 学不到不使用 box 实现这个特性的方法
> 3. doesn't necessarily carry over to other mutation operations
> 4. 更重要的是，不能让我们获得这些洞见
> 
> 不过理解它是很有用的，而且考虑到你在实现自己的语言的时候可能会觉得采用该种策略是个好主意。因此，试试使用这种策略实现一下我们的解释器。还需要 **store-passing style** 吗？为什么？
...
此后，我们使用**变量**表示在其作用域内值可以发生变化的标识符，而值不能变化的使用**标识符**表示。在对一个符号属于哪种情况存疑的时候，安全一点，我们就称之为“变量”；如果这种区分不太重要时，我们可能使用其中任意一个。不要被这些名词搞得头大，重要的是理解它们的区别。

> 如果一个标识符被绑定到一个 `box`，那么它将总是被绑定到同一个 `box` 值。会发生改变的是 `box` 的内容，而其绑定的 `box` 不会改变。

### 语法

大部分语言使用 `=` 或者 `:=` 表示赋值，Racket 中不一样：使用 `set!` 表示变量赋值。
...
考虑该语句，我们是想在任何意义上对字符串 `s` 进行改变吗？当然不是。我们显然只是想改变 `o` 表示的值，以使得后面程序中 `o` 被求值时得到的是这个新的字符串对象。
...
然后就没有然后了！这个解释器已经完成了。所有麻烦事已经在之前实现 *store-passing style* 时做完了。本节代码在[这里](./src/chap8.2.rkt)。

## The Design of Stateful Language Operations

尽管大部分语言至少包含我们学习的两种状态之一，但是它们的选入不应该被当做一件微不足道或者说理所当然的事。另一方面，状态的引入带来了一些显著的好处：

* 状态提供了某种形式的模块化。拿我们上面实现的解释器为例，由于没有显式的状态操作：
  * 为了传递 store，需要给函数的参数和返回值添加 store
  * 而且需要为每个状态的改变者和消费者函数进行这种改变
  可以将编程语言中的状态理解为在所有函数隐式流动的的参数和返回值。它使得不同函数可以进行“超距”通信，中间过程无需知晓这种通信。
* 状态使得我们可以以一种更为简洁直观的方式构造动态，环形的数据结构
...
另一方面，状态也给程序员和处理程序的程序（如编译器）带来不少麻烦。其中一个是“别名（aliasing）”，后面会讨论到。另一个是“引用透明性（referential transparency）”，也是希望后面能讨论的。最后，上面我们说过状态提供了一种形式的模块化，然而，换个角度去看那个讨论，我们可以进行反向通信，而中间过程同样无法监控这种通信。某些情况下（特别是安全和分布式系统中），这种反向通道可能造成不可预知的安全威胁。

由于没有更好的方案，可能我们需要在引入 mutation 操作时要特别谨慎。例如，在 Standard ML 中没有变量，因为它被认为不是必要的，但是包含了等同于 `box` 的东西（叫做*引用(refs)*）。你可以很容易的通过 `box` 模拟变量，尽管可能会由于使用不慎导致比变量别名更严重的问题。

作为回报，开发者得到非常有表现力的类型：除非包含引用，每个数据结构都可被认为是 immutable 的，而引用的存在也可以作为对于开发者和程序（如编译器）的提醒，即它引用的值可能会发生改变。
...
现在，试想程序以下面说的这种方式执行。当实参为变量时——它在内存中在有个地址——我们不再为该值重新分配地址，而是直接使用变量原来的地址。于是现在形参和实参指向的是内存中的同一块地址：即它们为变量别名（variable aliases）。这样在函数内部进行的改变会泄漏到调用者的上下文中；上面的这段例子程序也将计算得到 3 而不是 5。这被成为传引用调用（call-by-reference）的参数传递策略。

在很长的一段时间里，这种被认为是一种很好的特性。使用它可以进行一些很有用的抽象，比如 swap 函数，你可以调用该函数交换调用者上下文中两个变量的值。然而，这种特性的劣势远大于优势：

* 粗心的程序员可能会无意间创建了一个别名变量，然后修改变量的值，如果没有触发特别的异常的话，调用者可能永远不会注意到这种错误。
* 一些人认为这种策略效率较高：他们觉得与传引用相对的策略就是需要拷贝大量数据。但是，传值（call-by-value）和仅传递数据结构的地址是不冲突的。仅在下列情况夏需要拷贝数据：a)数据结构是不可变的，b)不希望调用者改变实参值，c)语言本身没有提供不可变相关支持。
* 它可能导致不一致性及模块化的丧失。例如，考虑下面的程序：
  ```scheme
  (define (f g)
    (let ([x 10])
      (begin
        ...
        ...)))
  ```
  如果允许传引用调用的话，程序员将不能仅看看上面的代码就确定 `(g x)` 调用后 `x` 的值。

如果非要允许传引用调用的话，至少需要让调用者决定是传引用——让在函数内部的形参共享传入的实参的内存——还是不使用传引用。然而即使使用这种方式还是会招致一个与之对称的问题，现在被调用者不知道它的参数到底是不是个别名了。传统的顺序程序中，这可能还不是个问题，但是如果程序是可重入的，被调用者将面临前面提到的调用者面临的那些窘境。

是时候考虑一下引入这些东西是否值得了。如果调用者想要执行一个 mutation 操作，可以给被调用者传递 `box` 值。`box` 作为调用者接受或者说允许被调用者进行 mutation 操作的信号，执行结束后，调用者再从抽取出 `box` 中的值。当然这样我们就不能写出一个很简洁的 `swap` 函数，但是为了真实世界软件工程的考虑，这点小代价还是花的起的。
